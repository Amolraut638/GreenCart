...
const Cart = () => {
    ...
    const getCart = () => {
        let tempArray = []  //this array contains all the products available in the cart
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.addresses.length > 0) {
                    setSelectedAddress(data.addresses[0]) //find first address using index 0
                } else {
    ...
    //this is the useEffect which is used to execute the above getCart function
    useEffect(() => {
        if (products.length > 0 && cartItems) {
            getCart()
        }
    }, [products, cartItems]) /* when these two are changes then this function executed */

    //for calling getUserAddress funtion
    useEffect(()=>{
        if (user) {
            getUserAddress();
        }
    ...
    return products.length > 0 && cartItems ? (
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                <h1 className="text-3xl font-medium mb-6">Shopping Cart <span className="text-sm text-primary">{getCartCount()} { getCartCount() > 1 ? "items" : "item" }</span></h1>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    {/* here i have wrapped e in the brackets  */}

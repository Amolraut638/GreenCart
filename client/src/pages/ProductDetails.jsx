...
const ProductDetails = () => {
    ...
    const product = products.find((item) => item._id === id); /* id recieving from params */

    useEffect(()=>{
        if (products.length > 0) {
            let productsCopy = products.slice();
            productsCopy = productsCopy.filter((item)=> product.category === item.category)  /* we get product list for perticular category */
            setRelatedProducts(productsCopy.slice(0,5));
        }
    },[products])  /* when the product is changing then this function will be executed */

    
    useEffect(()=>{
        setThumbnail(product?.image[0] ? product.image[0] : null)
    },[product]) /* when individual product is availabel */
    ...
    return product && (
        ...
        <div className="mt-12">
            <p>
                ...
                <span className="text-primary"> {product.name}</span>  {/* we are not going to click on this so we dont apply to={} property here */}
            </p>

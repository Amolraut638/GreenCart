...
const ProductCard = ( {product} ) => {
    ...
    //currency is coming from app context so this function we have to create        
    const { currency, addToCart, removeFromCart, cartItems, navigate } = useAppContext();

    //if (!products) return <div className="text-red-500">Product not found</div>; // Safe fallback
        ...
        <div onClick={()=> { navigate(`/products/${product.category.toLowerCase()}/${product._id}`); scrollTo(0,0) }} className="border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white min-w-56 max-w-56 w-full">
            ...
            <div className="text-gray-500/60 text-sm">
                ...
                <div className="flex items-center gap-0.5">
                    ...
                    <p>(4)</p>  {/* we dont have dummy data for rating so we did this hard coded value */}
                </div>
